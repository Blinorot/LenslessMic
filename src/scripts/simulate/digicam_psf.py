...
from lensless.utils.io import get_dtype, load_psf, save_image
...
def digicam_psf(config):
    ...
    if config.digicam.vertical_shift is not None:
        if config.use_torch:
            ...
            mask = np.roll(
                mask, config.digicam.vertical_shift // config.digicam.downsample, axis=1
            )
    ...
    if config.digicam.horizontal_shift is not None:
        if config.use_torch:
            mask = torch.roll(
                mask,
                config.digicam.horizontal_shift // config.digicam.downsample,
                dims=2,
            )
        else:
            mask = np.roll(
    ...
    if psf_meas is not None:
        ...
        # modify colors a bit to see better
        hsv = mcolors.rgb_to_hsv(psf_meas)
        # Shift hue (e.g. rotate color wheel slightly)
        hsv[..., 0] = (hsv[..., 0] - 0.16) % 1.0
        # Convert back to RGB
        psf_meas_hue = mcolors.hsv_to_rgb(hsv)

        psf_meas_norm = psf_meas_hue[0] / np.max(psf_meas)
        psf_in_np_norm = psf_in_np / np.max(psf_in_np)

        plt.figure()
        plt.imshow(psf_in_np_norm, alpha=0.9)
