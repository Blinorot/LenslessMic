...
from lensless.hardware.sensor import SensorOptions, SensorParam, sensor_dict
from lensless.hardware.utils import check_username_hostname
from lensless.utils.image import print_image_info, rgb2gray
...
@hydra.main(
    ...
    config_name="lensless_demo",
)
def liveview(config):
    sensor = config.capture.sensor
    assert (
        sensor in SensorOptions.values()
    ), f"Sensor must be one of {SensorOptions.values()}"
    ...
    if config.capture.awb_gains:
        pic_command += (
            f" awb_gains=[{config.capture.awb_gains[0]},{config.capture.awb_gains[1]}]"
        )
