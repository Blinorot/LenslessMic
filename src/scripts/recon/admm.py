...
def admm(config):
    if config.torch:
        try:
            ...
            raise ImportError(
                "Pytorch not found. Please install pytorch to use torch mode."
            )
    ...
    if not config.admm.unrolled:
        ...
        recon = UnrolledADMM(
            psf, pre_process=pre_process, post_process=post_process, **config.admm
        )
    ...
    if save:
        if config.admm.unrolled:
            ...
            if res[2] is not None:
                ...
                ax = plot_image(
                    pre_post_process_image, gamma=config["display"]["gamma"]
                )
        ...
        if config.input.original is not None:
            ...
            lpips_func = lpip.LearnedPerceptualImagePatchSimilarity(
        ...
        if config.admm.unrolled and recon.pre_process is not None:
            ...
            img = res[0].cpu().numpy()
            np.save(
                plib.Path(save) / "final_reconstruction_no_preprocessing.npy", img[0]
            )
