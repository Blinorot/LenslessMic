...
from src.transforms.xcodec.quantization import ResidualVectorQuantizer
from src.transforms.xcodec.semantic_module import Decoder, Encoder
...
class SoundStream(nn.Module):
    def __init__(
        ...
        n_filters: int = 32,
        D: int = 256,
        target_bandwidths: Sequence[Union[int, float]] = [0.5, 1, 1.5, 2, 4],
    ...
    def video_to_audio(self, codec_video, return_codes=False):
        ...
        audio = self.decoder_2(quantized_acoustic)

        if return_codes:
            # codes here are N_qxBxT
